...
未勾选项目是计划目标，不代表原服务可以被百分之百复刻。模型、Harness、播放器、渲染器和 3D 同步器都采用可替换设计，用户可以在设置中选择具体实现，后续可以持续改进而不必重写游戏接入层。总体模块化约定见 [`docs/architecture.md`](./docs/architecture.md) 和 [`docs/requirements.md`](./docs/requirements.md)。
