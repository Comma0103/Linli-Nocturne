...
Phase 0–3 的基础功能已完成，Phase 4 的 4-1 至 4-4 已完成，项目当前仍是开发版。Phase 4-5 至 4-7、定制演奏与 3D 表现、视频回信自动生成和最终发行形态仍在后续路线中。

## 当前功能

以下内容只描述当前版本已经提供和仍未提供的能力；实现细节、设计取舍与验收记录见 [`docs/`](./docs/)。

### 已实现

#### 信件与回信

* [x] **信件规则与可靠处理**：支持每日额度、发送延迟、排队、状态流转、失败重试和重启恢复，并提供可配置的旁路规则。
* [x] **可替换的回信生成链路**：支持离线人格引擎、外部 OpenAI 兼容 API、本地 OpenAI 兼容模型，以及可组合的 Persona、Harness 和记忆实现。
* [x] **Persona、Harness 与持续记忆**：提供 Olivia-lin 人格素材、Fusion Harness、SQLite 持久记忆、关系账本、历史检索和执行元数据。
* [x] **玩家档案与数据迁移**：按玩家隔离信件和记忆，支持改名、切换、查看、清空、导出和导入。
* [x] **游戏内文字回信**：支持在 Steam 客户端 `0.0.9.627` 中查看离线回信和在线模型回信。
...
* [x] **原生 WebPlayer 播放/演奏接管**：用户曲目和歌单曲目可以进入原生播放/演奏流程；当前上传 MIDI 使用音频媒体，演奏画面为黑屏，不包含 3D 人物演奏画面。
* [x] **本地试听**：支持上传曲目和已下载预设曲目的本地试听；试听播放器与底部演奏栏彼此独立。
...
* [x] **视频回信资产管理**：支持已有视频资产的导入、格式检查、保存、播放、替换、删除和任务状态管理；不包括视频回信自动生成。
...
* **音乐来源**：设计并实现合法的外部歌单/歌曲导入适配器，接入统一曲目表示。
* **演奏体验**：完善宽松演奏、MIDI 到演奏媒体的技术底座，以及后续的手指、琴键、镜头和动作同步。
* **视频回信**：实现独立的 `VideoGenerator`、即兴创作上下文和可替换的视频生成/渲染 provider。
* **产品交付**：完善普通用户 GUI 设置、安装器、诊断、备份恢复和发行流程。
* **质量与兼容性**：补充其他客户端版本、更多媒体格式、性能和可访问性验证。
