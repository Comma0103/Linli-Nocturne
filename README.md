...
- [x] **信件规则**：默认遵循原游戏每日最多 3 封、每封延迟 5 分钟的规则，并提供 bypass 开关；信件支持排队、状态流转、失败重试和最大尝试次数。
- [x] **文字回信**：支持离线 fallback、OpenAI 兼容外部 API、本地模型，以及统一的 Persona/Harness 组合；外部模型失败时可按配置回退到离线实现。
- [x] **信件记忆**：提供可选 MemoryProvider，限制记忆条数、单条长度和上下文长度，默认关闭。
- [x] **Steam 信件体验**：已在客户端 0.0.9.627 验收离线回信，以及 DeepSeek + Persona + OliviaSoul Harness 的真实回信。
- [x] **MIDI 与时间轴**：支持标准 MIDI 文件解析、音符、Tempo、延音踏板和时间轴清单，并可生成本地 WAV/MP4 媒体任务。
- [x] **MIDI 网关**：支持上传、任务创建、状态轮询、取消、删除、媒体读取、统一 RenderJob 字段、SQLite 任务恢复和用户曲目游标分页，兼容原版客户端的请求与响应字段。
- [x] **曲库与歌单**：基于 SQLite 保存用户曲目和歌单，并提供离线曲库入口；已接入的客户端可显示本地生成曲目。
- [x] **可插拔模块**：Provider、Harness、Persona、AudioRenderer、GamePlaybackAdapter、RenderJob 和 ModuleRegistry 均通过统一接口或适配器连接，具体实现可由配置选择。
- [x] **视频回信资产**：支持视频导入、格式检查、保存、播放、替换、删除和处理中/成功/失败状态。
- [x] **安全接入与恢复**：安装前执行版本白名单和 SHA-256 基线校验，使用游戏目录外备份，并支持已知补丁的回滚与修改检测。
- [ ] **原生播放/演奏接管**：上传 MIDI 尚未让原生 WebPlayer 切换到本地媒体并进入演奏流程（`LINLI-PLAY-001`）。
- [ ] **外部音乐来源**：网易云、QQ 音乐等歌单/歌曲导入，以及宽松演奏模式。
- [ ] **后续创作与发行**：视频自动生成、即兴作曲、3D 手指/琴键/镜头同步、最终 GUI 设置、普通用户安装器和发行版。
