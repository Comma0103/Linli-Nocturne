...
- **MIDI 网关**：已具备符合原版客户端响应契约的本地上传、任务创建、结果轮询、WAV 媒体读取、统一 RenderJob 字段、SQLite 任务元数据持久化，以及用户曲目游标分页；协议回归测试覆盖上传预检、下划线字段和数字任务状态。游戏 HTTPS 页面使用独立的本地 HTTPS 媒体端点，避免浏览器混合内容拦截。
