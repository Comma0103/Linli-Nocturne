...
原版主脚本把信件状态和回信类型定义为数字枚举。领域层继续使用可读的字符串状态，但 `/toy/letter/list` 和 `/toy/letter/detail` 必须在兼容边界转换：

| 领域状态 | 客户端 `letterStatus` | 含义 |
| --- | ---: | --- |
| `pending` | `1` | 等待可处理 |
| `processing` | `3` | 模型处理中 |
| `replied` | `4` | 已有回信 |
...
原版 `replyType` 中 `0` 是无回信、`1` 是文字回信；前端把其他非零类型交给视频回信组件。本地导入的 MP4 回信使用 `2`，文字回信继续使用 `1`，本地接受的信件使用 `auditStatus: 2`。这组数字只属于 `src/gateway/letter-compat.js` 的游戏适配边界，不能泄漏回领域服务或 SQLite。此前网关误返回字符串 `replied`，会导致原版前端无法建立回信展示对象，表现为一直显示“林离回信中…”。
...
前端开关恢复和本地 MIDI 后端已分别实现并验收。2026-09-09，已接入客户端的上传、生成、曲库/歌单演奏及试听通过 Phase 4-1 至 4-4 验收；当前上传结果为音频 MP4 和黑屏演奏，动态人物画面与精确手指同步仍属 Phase 5。任务契约保留 `timeline`、`handTracks`、`cameraTracks` 和 `motionTracks` 扩展方向，不代表已有实际 3D 产物。

预设试听复用前端音频播放器，只把其来源映射到本地缓存接口；底部演奏栏仍由原生 WebPlayer 驱动。`f51d630` 进一步在歌单加入/读取路径补齐预设元数据，并在封面加载失败时复用音符占位；该补丁已应用到受控 Steam `0.0.9.627` 样本，用户确认旧、新预设均可显示和演奏。原版一次安装与已知开发样本的增量补丁尚未完全统一，详见[安装整合边界](./original-installation.md#当前交付边界2026-09-09)，不能只凭这次受控样本验收断言新机器安装成功。
