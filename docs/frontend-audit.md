...
原版 `replyType` 中 `0` 是无回信、`1` 是文字回信；本地接受的信件使用 `auditStatus: 2`。这组数字只属于 `src/gateway/letter-compat.js` 的游戏适配边界，不能泄漏回领域服务或 SQLite。此前网关误返回字符串 `replied`，会导致原版前端无法建立回信展示对象，表现为一直显示“林离回信中…”。
