...
- **模型消息边界**：普通来信由 OpenAI 兼容 provider 组装 `currentLetter/history` 和时间上下文。Harness 与记忆整理通过显式 `system` 提交已组装的任务，其 `prompt` 原样进入 user message，不能再包成一封来信；否则预检会把初始化账本等任务指令误判为用户提示注入。该处理不跳过预检、质量检查或失败重试。
