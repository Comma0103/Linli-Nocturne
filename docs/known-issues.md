...
本地服务入口、信件 Worker、provider 选择和兼容网关已经有自动化测试。2026-09-07 用户实际观察到信件已经发送，但界面仍显示“林离回信中…”。只读审计发现网关曾把领域字符串状态直接返回给客户端，现已改为 0.0.9.627 所需数字枚举并补回归测试；仍需用户重新打开已有信件确认正文显示，再记录为实机通过。自动化健康检查不等于游戏界面验收。
