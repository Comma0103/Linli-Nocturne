...
`exp/phase3-persona-harness-fusion` 已实现分层摘要、关系账本、原文检索和数据迁移，自动化通过；最新称呼与持续记忆组合尚未完成 Steam 离线/在线复验。无模型离线可以保存/读取已有历史，语义摘要和关系推断仍需要模型。迁移包当前上限 256 MiB，密钥与外部运行时不随包搬迁。具体实施和复验项见[融合实验 §4.1](./phase3-exp-persona-harness-fusion.md#41-持续关系与长期记忆2026-09-08)。
