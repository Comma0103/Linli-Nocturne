...
## Phase 3-7：Steam 游戏界面实机验收尚待用户报告

本地服务入口、信件 Worker、provider 选择和兼容网关已经有自动化测试。仍需在退出游戏、完成只读基线检查并按受保护安装流程接入后，由用户实际在 Steam 界面发送信件、观察排队和文字回信，再记录为实机通过。自动化健康检查不等于游戏界面验收。
