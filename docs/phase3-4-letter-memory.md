...
本轮实验分支使用 `olivia-soul.sqlite` 持久记忆完成在线写信和跨重启读取复核；执行记录确认记忆上下文来自当前玩家且没有 fallback。历史记录中“默认有限记忆”的描述属于本里程碑原始设计，当前模板默认和实际验收组合以 [user-config.md](./user-config.md) 及融合实验为准。
