...
> 本文保留最初有限记忆里程碑。下文“默认关闭”和只保留近期窗口属于历史设计。当前模板已默认开启 `olivia-soul.sqlite`，新增分层长期记忆、关系账本和自动迁移；实现、验收及 Steam 复验见[融合实验 §4.1](./phase3-exp-persona-harness-fusion.md#41-持续关系与长期记忆2026-09-08)。这些改动已随 `phase3-letter-fusion-v1.0.0` 合入 `main`。

## 本轮定位

本轮让回信 provider 可以参考之前的有限对话，同时把记忆做成可替换模块。默认核心流程不依赖某个数据库、模型或第三方 Harness；视频回信和 Phase 4 的 `LINLI-PLAY-001` 不在本轮实现。模块选择现在由统一设置模型和可读配置向导承载，最终 App 图形设置界面仍属于后续设置/发行阶段。
...
本轮只实现有限对话记忆和统一接口，不实现自动摘要、向量检索、人格长期档案或视频回信。记忆开关和 provider 选择已经可以通过 `ModuleSettings` 与配置向导调整；最终 App 图形设置界面将在后续设置/发行阶段接入同一模型。

## 2026-09-11 回归说明

2026-09-11 在线验收使用 `olivia-soul.sqlite` 持久记忆完成在线写信和跨重启读取复核；执行记录确认记忆上下文来自当前玩家且没有 fallback。历史记录中“默认有限记忆”的描述属于本里程碑原始设计，当前模板默认和实际验收组合以 [user-config.md](./user-config.md) 及融合实验为准。
