...
Phase 3 与 Phase 4 没有“必须先完成 Phase 3 才能修复 MIDI”的技术依赖；只是按项目节奏先完成信件，再恢复音乐体验。当前仓库已经完成 Phase 3 首个信件可靠性里程碑，后续可继续信件的真实 provider、后台 worker、视频回信导入；不要让 `LINLI-PLAY-001` 重新打断这些工作。
...
Phase 3 首个信件可靠性里程碑已经在仓库中完成；先检查其提交和测试，不要重复实现。接下来继续真实 provider、后台 worker、视频回信导入等 Phase 3 任务。不要让 `LINLI-PLAY-001` 阻塞 Phase 3，也不要把它当作 Phase 3 的技术前置。进入 Phase 4 时，再按本交接文档的边界完成用户上传 MIDI 的预览、媒体任务、曲库和播放/演奏验收；原生问题必须先做只读证据调查。每一步先写需求/设计和验收标准，再做小步代码修改、单元测试、网关冒烟测试和文档同步。任何涉及真实游戏目录的操作都必须先退出游戏、只读检查版本/基线、使用游戏目录外备份，并在写入前确认计划可应用；不要直接修改用户安装来试错。
