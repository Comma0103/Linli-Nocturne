...
Phase 3 的信件可靠性、真实 provider、后台 worker、有限记忆、视频资产和总体验收已经在仓库中完成；本地服务启动入口也已具备。剩余的 Phase 3-7 是 Steam 游戏界面实机验收，必须由用户实际启动游戏、点击并报告观察结果后才能记录完成。完成后再进入 Phase 4，按本交接文档的边界完成用户上传 MIDI 的预览、媒体任务、曲库和播放/演奏验收，并把网易云/QQ 音乐等外部来源作为可插拔导入适配器规划。不要让 `LINLI-PLAY-001` 影响 Phase 3，也不要在没有原生证据时修改它。每一步先写需求/设计和验收标准，再做小步代码修改、单元测试、网关冒烟测试和文档同步。任何涉及真实游戏目录的操作都必须先退出游戏、只读检查版本/基线、使用游戏目录外备份，并在写入前确认计划可应用；不要直接修改用户安装来试错。
