...
最终愿景包括：恢复信件入口、文字回信、视频回信、用户上传 MIDI、本地音频与游戏内曲库、即兴创作，以及可扩展到 3D 手指/琴键/镜头/动作同步。Phase 2 只负责安全接入、网关契约、基础曲库与原版客户端连接；完整的用户音乐体验属于 Phase 4，真正的 3D 演奏和手指/琴键/镜头/动作同步属于更后续阶段。
...
Phase 2 已达到“接入基础完成并可交接”的状态，不是“所有最终愿景完成”，也不是可直接面向普通用户发布的 release。领域服务、协议、补丁规划、备份回滚、离线入口和官方预设曲目的 Steam 实机链路已有实现与测试。用户上传曲目的完整预览/播放/演奏体验原本属于 Phase 4；我们曾在 Phase 2 提前做纵向验证，因此暴露出 `LINLI-PLAY-001`。它现在应标记为 Phase 4 的跨层阻塞，而不是 Phase 2 或 Phase 3 的前置条件；本阶段暂时封存，不再用猜测式修改消耗测试时间。
...
- [x] 当前自动化测试基线：Phase 3 总体验收后 `pnpm test` 通过；本次加入本地服务启动入口后，实际通过项数以最新命令输出为准。

### 2.2 明确未完成或不能宣称完成

- [ ] **Phase 4 跨层阻塞 `LINLI-PLAY-001`：上传曲目无法接管原生 WebPlayer。** 前端发出带完整 `song` 对象的本地曲目 `play`，原生桥也收到；但本地媒体没有成为当前媒体源，旧预设曲目继续产生时间事件，桌面不进入演奏且没有本地声音。上传短 MIDI、长 MIDI、重启和 `.mp4`/HEAD/Range 兼容尝试都未解决。不能把 `songPlayStart` 埋点或“停止”按钮当作成功证据。该问题不阻塞 Phase 3。
...
- [x] Phase 3 信件后台处理已完成：`LetterWorker` 定时领取待处理信件，SQLite 原子领取避免重复处理，过期 `processing` 租约会在重启后回到 `pending` 或进入 `failed`；手动 `POST /letter/process` 仍保留用于诊断。真实模型仍需用户自行配置，不能宣称所有机器已经启用自动文字回信。
- [x] Phase 3 有限记忆已完成：`MemoryProvider` 默认使用 SQLite 有界记忆，也支持显式关闭和可替换实现；记忆读取/写入失败不会让已生成的回信失败。
- [x] Phase 3 视频回信资产流程和总体验收已完成；仍未完成的是视频自动生成、即兴作曲、真实 3D 手指/琴键/镜头/动作同步、普通用户安装器和 release。
...
- `src/app/local-app.js`、`scripts/start-local-service.mjs`：开发版把模块设置、SQLite、LetterWorker、兼容网关和媒体服务一次性启动起来；默认离线 fallback，不保存密钥。
...
Phase 3 的总览和详细文档索引见 [`phase3.md`](./phase3.md)。

Phase 3 与 Phase 4 没有“必须先完成 Phase 3 才能修复 MIDI”的技术依赖；只是按项目节奏先完成信件，再恢复音乐体验。Phase 3 的信件可靠性、真实 provider、后台 worker、有限记忆、视频资产和总体验收已经在仓库中完成；先检查其提交和测试，不要重复实现。下一步进入 Phase 4，按本交接文档的边界完成用户上传 MIDI 的预览、媒体任务、曲库和播放/演奏验收。不要让 `LINLI-PLAY-001` 阻塞已完成的 Phase 3，也不要在没有原生证据时修改它。每一步先写需求/设计和验收标准，再做小步代码修改、单元测试、网关冒烟测试和文档同步。任何涉及真实游戏目录的操作都必须先退出游戏、只读检查版本/基线、使用游戏目录外备份，并在写入前确认计划可应用；不要直接修改用户安装来试错。
...
4. 已完成：文字回信和视频回信资产流程均有网关端到端测试，视频导入、替换、删除和状态页已覆盖。
...
Phase 2 已完成接入基础：网关、SQLite/RenderJob、MIDI 解析和 WAV/MP4 媒体基础、用户曲目分页、版本白名单、SHA-256 基线、外置备份、回滚、前端/DLL 受保护补丁、离线曲库入口，以及官方预设曲目的 Steam 实机演奏验收。完整用户上传音乐体验属于 Phase 4。当前自动化基线是 pnpm test，先运行测试并检查 git status；不要把 work/、日志、数据库、媒体、游戏资源或专有 DLL 提交。

必须保留并明确标记的 Phase 4 跨层阻塞是 LINLI-PLAY-001：本地上传 MIDI 可以上传、生成、显示和加入音乐桌面，但点击“演奏”时原生 WebPlayer 没有切换到本地媒体，旧预设曲目继续发送时间事件，桌面不进入演奏且没有本地声音。官方预设曲目正常。此前尝试过短/长 MIDI、重启、localhost、.mp4、HEAD/Range、TOD1200/TOD1730/TOD2000 和恢复原生 song 对象；不要把它当作 Phase 3 任务，也不要再次盲目改 video/audio 或要求用户重复无效测试。只有拿到原生 WebPlayer 的只读反汇编、CEF 媒体事件或完整媒体请求证据后，才创建单独诊断任务。

本阶段遗留的其它边界：安装器还不是最终发布安装器（缺进程锁、事务日志、安装后完整验证和卸载命令）；真实模型是否可用取决于用户配置；视频自动生成、即兴作曲、3D 同步、普通用户安装器和 release 尚未完成；只验证过客户端 `0.0.9.627`。不要宣称最终愿景已经完成。

Phase 3 的信件可靠性、真实 provider、后台 worker、有限记忆、视频资产和总体验收已经在仓库中完成；本地服务启动入口也已具备。原始交接快照中“剩余的 Phase 3-7 Steam 游戏界面实机验收”已由后续用户实机验收完成；当前状态见本文件后文及 [Phase 3-7 验收记录](./phase3-7-steam-acceptance.md)。现在进入 Phase 4，按本交接文档的边界完成用户上传 MIDI 的预览、媒体任务、曲库和播放/演奏验收，并把网易云/QQ 音乐等外部来源作为可插拔导入适配器规划。不要让 `LINLI-PLAY-001` 影响已完成的 Phase 3，也不要在没有原生证据时修改它。每一步先写需求/设计和验收标准，再做小步代码修改、单元测试、网关冒烟测试和文档同步。任何涉及真实游戏目录的操作都必须先退出游戏、只读检查版本/基线、使用游戏目录外备份，并在写入前确认计划可应用；不要直接修改用户安装来试错。
...
2026-09-07 的 Steam 实机观察显示，数据库已经生成文字回信，但游戏仍显示“林离回信中…”。只读审计确认 0.0.9.627 前端要求数字信件状态，网关此前返回领域字符串；现已在 src/gateway/letter-compat.js 适配边界修复并补回归测试。服务重启后需由用户重新打开已有信件确认正文显示，Phase 3-7 在此之前仍未完成。
