...
>
> 2026-09-09 核查时，远端 `main` 与本地 HEAD 均为 `ad7602df`。随后 Phase 4-3 通过只读反汇编定位原生 TOD 与本地 UGC 文件契约，并在服务侧完成修复；`pnpm test` 130/130 通过，Phase 4-4 Steam 0.0.9.627 实机上传、生成、播放验收已通过。Phase 3 融合已通过 `phase3-letter-fusion-v1.0.0` 合入；原交接快照 `615ff80` 仅作历史参考。接手时仍须核对分支、提交、工作区和测试，不把本地验收记录当作已推送证明。
...
现在继续推进 **Phase 4-2** 的客户端歌单确认，并准备后续 4-5 至 4-7。Phase 4-1、4-3、4-4 已完成；先做现状核对和本轮需求、设计、验收标准，再实施最小增量修改；不要重做 Phase 0–3，也不要一次性重写整个 Phase 4。
...
| Phase 4 | 完整音乐体验 | Phase 4-1、4-3、4-4 已完成；Phase 4-2 仍待客户端歌单操作确认，4-5 至 4-7 待开发 |
...
| 媒体格式是否一致 | 编码器声明的扩展名和 MIME 随成功任务保存，旧记录按原文件后缀恢复；更换编码器后，旧文件仍保持自己的后缀和 GET/HEAD/Range 响应类型。第三方输出的独立格式检查仍待补齐；这些结果不能解释原生 WebPlayer 未切歌。 |
| 取消与恢复的深度 | 已有 `queued/processing` 后台处理、MIDI 输入持久化、协作式取消及重启恢复；数字状态映射遗漏已修复，并通过 130/130 自动化测试。第三方 Renderer 是否支持强制中断仍取决于其实现。 |
...
旧文档还存在局部历史表述，例如 module-settings 中的“人格资产不会复制进仓库”“所有设置 JSON 都不能保存密钥”“默认 data 在项目外”；known-issues 尾部的“可以先完成 Phase 3”也曾是旧快照中的表述，现已改为 Phase 3 已完成。应按本交接、当前模板和源码理解；进入相关修改时同步纠正，不按这些旧句子撤回已内置资产或重复 Phase 3。本次已同步 README、Phase 3 验收记录、已知问题和第三方复用清单。
...
| Phase 4-2：曲库、歌单和模块选择 | 补齐曲目管理、歌单与播放器/Renderer/Encoder 选择，让设置真正影响运行结果。设计与验收见 [Phase 4-2 文档](./phase4-2-library-playlist-and-module-entry.md)。必要的简单界面或易懂入口随功能交付，不能全部推到最终发行再做。 |
...
| Phase 4-7：完整音乐体验总验收与交接 | 汇总前六轮的音乐设置、曲库歌单、原生证据、Steam 播放/演奏、外部来源和宽松演奏结果，完成 Phase 4 总体验收和下一阶段交接。 |
