...
下一步推进 **Phase 4-5** 的外部歌单与歌曲导入调研、设计。Phase 4-1 至 4-4 已完成；先盘点已有 Importer、MusicService、曲目表示和媒体链路，再核查来源导出方式、成熟实现、许可和最小接入范围，写好设计与验收标准后实现。4-6 宽松演奏和 4-7 总验收仍待完成；不要重做已有功能，也不要一次性重写整个 Phase 4。
...
4. 对照第 4 节和 Phase 4 各里程碑的最新验收记录，区分“已有能力”“尚未完整接通”“未来扩展点”，确定下一个可验证增量，不重复已经完成的修复。
...
| Phase 4 | 完整音乐体验 | Phase 4-1 至 4-4 已完成；下一步为 4-5 调研与设计，4-5 至 4-7 尚未完成 |
...
- 本地曲目已能在离线曲库显示、加入歌单、去重和移除，名称与日期字段已修正；从“我的上传”和歌单均能进入演奏。预设及上传曲目试听已复验，底部演奏栏不跟随独立试听更新；预设缓存命名范围见 Phase 4-2。
- AudioRenderer、Renderer 注册表、GamePlaybackAdapter、Encoder 和统一 RenderJob 已存在，不要从零重建。
- 版本白名单、SHA-256 基线、外置备份、受保护补丁、回滚和修改检测已实现。只验证过客户端 `0.0.9.627`；最终安装器仍缺全部离线 MIDI 增量补丁的统一整合、进程锁、事务日志、安装后完整验证和卸载命令，详见[游戏接入](./original-installation.md#当前交付边界2026-09-09)。本地启动包装器只改善服务启动，不替代完整安装器。
...
- 启动命令为 `.\scripts\start-local-service.ps1`（也可双击 `scripts/start-local-service.cmd`），默认显示 `http://localhost:27149`；健康检查应在另一个终端执行。配置修改后需重启服务。脚本会自动探测常见 Node.js 安装位置，避免依赖当前 PowerShell 的 PATH。
- 默认运行目录是仓库根目录下被忽略的 `data/`，可通过 `LINLI_DATA_ROOT` 指定其他位置。SQLite、媒体主副本和 Harness 运行副本都必须在 Steam 游戏目录之外；并非必须在代码仓库之外。原生演奏另需把生成媒体镜像到游戏配置的 `songStoragePath`，由现有 NativeUgcMediaStore 负责，不能因主数据目录约束而移除必要的播放镜像。
...
以下核查最初基于 `6d3d10c`，已按 2026-09-09 本地增量更新。配置、媒体契约和任务生命周期应同时核对源码与对应验收记录，不因 README 的概括勾选而忽略运行入口的实际行为。
...
| 取消与恢复的深度 | 已有 `queued/processing` 后台处理、MIDI 输入持久化、协作式取消及重启恢复；数字状态映射遗漏已修复。Phase 4-2 收尾完整测试为 133/133；第三方 Renderer 是否支持强制中断仍取决于其实现。 |
...
历史阶段文档中的“当时待验收”“恢复调查”等表述只用于还原旧证据；Phase 2 → 3 的旧 Prompt 不再是执行入口。当前状态以本文、README 和各里程碑最新验收记录为准，避免重复已完成的 Phase 3 或重新盲试已修复的原生问题。

## 5. Phase 4 及后续执行路线

### 5.1 里程碑不是严格串行

这里的“七轮对话”表示七个主要开发主题，不表示七个必须首尾相接的封闭阶段。4-1 是任务和媒体基础，4-2 在接口稳定后可以并行补齐曲库、歌单和模块入口；4-3 的原生 WebPlayer 证据调查可以与前两项并行，因为它主要读取原版样本、日志和调用链。4-4 需要前面有可测试的曲目和足够的原生证据，4-5 需要稳定的内部曲目表示，4-6 需要可用的播放入口，4-7 才汇总全部验收。

交叉推进不改变完成门槛：每轮必须标明依赖和本轮实际完成项，不能把局部代码或自动化测试通过写成整个里程碑完成；README 中的 checkbox 只在该里程碑整体验收通过后勾选。后续规划应使用这张依赖关系，而不是把编号理解成无条件的线性顺序。

| 里程碑 | 本轮要交付的用户能力和验收边界 |
| --- | --- |
| Phase 4-1：MIDI 预览与媒体任务 | 复用上传/解析/渲染代码，补齐普通用户可操作的预览与任务状态；验证媒体格式、轮询、取消、失败恢复和重启行为。自动化测试及 Steam 上传、生成、播放流程已通过。先核对第 4 节，不重新造 MIDI 解析器。 |
| Phase 4-2：曲库、歌单和模块选择 | 已完成曲库、歌单及试听验收；模块选择入口是用户配置文件和终端向导，真实启动与重启生效已验证。游戏内图形设置页属于 Phase 7。设计与验收见 [Phase 4-2 文档](./phase4-2-library-playlist-and-module-entry.md)。 |
| Phase 4-3：原生证据调查 | 已完成样本、日志、调用链和原生 UGC 文件契约核查，并完成服务侧修复；详见 [Phase 4-3 文档](./phase4-3-native-webplayer-evidence.md)。 |
| Phase 4-4：Steam 播放/演奏验收 | 已由用户在 0.0.9.627 中确认媒体切换、进入演奏桌面、听到本地声音及进度推进。 |
| Phase 4-5：外部音乐导入 | 下一步先做复用检查与来源能力调研，再确定最小导入设计。为网易云、QQ 音乐等提供可替换来源适配器，把可用曲目信息、音频或 MIDI 转成内部曲目表示，再进入曲库和播放；只有曲目信息不等于已有可播放媒体。 |
| Phase 4-6：宽松演奏 | 支持音频播放或近似 MIDI 驱动的演奏，不要求手指与音乐逐帧匹配；不能把普通播放标成精确 3D 演奏。 |
| Phase 4-7：完整音乐体验总验收与交接 | 汇总前六项的音乐设置、曲库歌单、原生证据、Steam 播放/演奏、外部来源和宽松演奏结果，完成 Phase 4 总体验收和下一阶段交接。 |

外部音乐只处理用户有权使用、可合法取得的内容，不绕过 DRM 或访问控制；框架本身不添加地域限制。来源服务不得写死进 MusicService。

Phase 4 保持上述七个开发主题，按依赖关系逐步推进；对话次数不作为完成标准。Phase 5 先做定制演奏与 3D 表现的现有实现调研、最小技术预研和技术底座；Phase 6 再基于这套底座做视频回信与即兴创作；Phase 7 完成普通用户安装器、最终 GUI、诊断恢复与发行。

每轮先写需求、设计和验收标准，再小步实现、测试、网关冒烟和文档同步。优先修复能独立复现的具体缺口；不扩大成整套新框架，不因原生播放阻塞停止所有可独立完成的音乐工作。

每轮先检查仓库、已内置的第三方项目及已有工具是否能直接使用或稍作调整。能复用时必须复用；只有明确指出现有实现无法满足的具体差距后，才新增代码，避免重复造轮子。

## 6. LINLI-PLAY-001：已关闭与回归检查入口

当前事实：

- 本地 MIDI 能上传、解析、生成、在“我的上传”显示并加入歌单。
- 2026-09-09，Phase 4-4 已确认原生 WebPlayer 切换到本地媒体、黑屏演奏、发声、进度推进和自然结束；Phase 4-2 后续确认歌单入口同样可演奏，预设及上传曲目试听正常。
- 旧预设曲目继续计时而本地曲目不发声，是修复前的历史表现，不是当前阻塞。当前上传产物为音频 MP4，不等于 Phase 5 的人物演奏视频。
- 历史上已尝试短/长 MIDI、重启、localhost、.mp4、HEAD/Range 和错误 TOD；Phase 4-3 已通过只读反汇编确认正确 TOD 为 `TOD12/TOD1730/TOD20`，并确认原生要求本地 UGC 文件存在。后续不再进行无证据的字段试错。

详细证据见 [known-issues.md](./known-issues.md)。其中日志文件名和时间是历史证据索引，不保证新机器存在这些本地文件。缺少日志时不得编造其内容。

若以后再次无法演奏，先检查任务的 `info.nativePlayback`、缓存盘可用性、目标文件和原生 TOD，再对照已完成的 [Phase 4-3 调查](./phase4-3-native-webplayer-evidence.md)。只有出现无法由现有契约解释的新证据，才继续原生调查；不要重复从零反汇编、盲改字段或重复要求已通过的实机测试。用户没有明确要求另起任务时，诊断工作项留在当前任务内。

独立网关缺口与原生回归应分别证明和修复。Mixed Content 警告或 `songPreviewStart` 埋点都不能单独判定试听成败；用户的实际发声确认与同时间窗日志应共同记录。该缺陷不再阻塞 Phase 4-5 的调研和设计。
...
自上次交接后，`85bdb94` 统一了交接命名；`88a33bb` 至 `6d3d10c` 调整了 README、用户流程、Phase 3 状态和 Phase 4-1 首轮音乐配置/媒体契约。这些文档要求是用户已明确确认的要求：
...
检查差异并取得本轮提交授权后小步提交，推送 `main`，核对远端分支与本地 HEAD，并确认工作区没有遗漏。遇到远程新提交先查明并整合，不强推。修改 README 时不再另造长期状态文档；后续按第 5 节及各里程碑最新验收记录推进。
