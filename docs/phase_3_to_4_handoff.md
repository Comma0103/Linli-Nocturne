...
| Phase 4-2：曲库、歌单和模块选择 | 已完成曲库、歌单及试听验收；预设加入歌单的元数据、封面占位、旧条目回填、重复加入和移除已在 `f51d630` 后完成 Steam 复验。模块选择入口是用户配置文件和终端向导，真实启动与重启生效已验证。游戏内图形设置页属于 Phase 7。设计与验收见 [Phase 4-2 文档](./phase4-2-library-playlist-and-module-entry.md)。 |
| Phase 4-3：原生证据调查 | 已完成样本、日志、调用链和原生 UGC 文件契约核查，并完成服务侧修复；详见 [Phase 4-3 文档](./phase4-3-native-webplayer-evidence.md)。 |
| Phase 4-4：Steam 播放/演奏验收 | 已由用户在 0.0.9.627 中确认媒体切换、进入演奏桌面、听到本地声音及进度推进；本轮又确认预设歌单回填后的播放和相关回归。 |
...
- 2026-09-11，预设歌单旧/新曲目的名称、时长、音符占位和演奏已由用户在补丁后的 Steam 客户端确认；三条合成视频回信、第四套在线写信和 MIDI `bypass` 也已完成本轮最终回归。
