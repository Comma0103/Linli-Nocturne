...
以下是对当前 `main`（`6d3d10c`）源码和测试的边界核查。已完成的配置与媒体契约以当前测试为准；未完成的任务生命周期缺口仍需按本轮设计实现，不因 README 的概括勾选而忽略运行入口的实际行为。
...
| 媒体格式是否一致 | 已完成首轮契约：编码器声明的扩展名和 MIME 会进入 `MidiJobService` 与网关，WAV/MP4 的后缀、响应类型、HEAD/Range 有回归测试覆盖；这仍不能解释原生 WebPlayer 未切歌。 |
