...
>
> 2026-09-09 核查时，远端 `main` 与本地 HEAD 均为 `8dfa16a`。后续本地增量修复了 MIDI 数字状态、真实启动入口歌单写入及编码器切换后的旧媒体格式，`pnpm test` 127/127 通过；Phase 4-3 已开始只读样本与日志核查，原生根因未定位。Phase 3 融合已通过 `phase3-letter-fusion-v1.0.0` 合入；原交接快照 `615ff80` 仅作历史参考。接手时仍须核对分支、提交、工作区和测试，不把本地验收记录当作已推送证明。
...
现在继续收尾 **Phase 4-1/4-2** 的验收，并按 **Phase 4-3** 文档推进只读调查。先做现状核对和本轮需求、设计、验收标准，再实施最小增量修改；不要重做 Phase 0–3，也不要一次性重写整个 Phase 4。
...
| Phase 4 | 完整音乐体验 | Phase 4-1 正在开发；音乐配置、媒体契约和任务生命周期已实现，待用户确认客户端轮询体验；上传 MIDI 原生播放仍有跨层缺陷 |
...
| 取消与恢复的深度 | 已有 `queued/processing` 后台处理、MIDI 输入持久化、协作式取消及重启恢复；数字状态映射遗漏已修复。仍需用户确认客户端轮询体验并完成恢复专项验收，不代表所有第三方 Renderer 都能强制中断或已支持自动重试。 |
...
| 里程碑 | 本轮要交付的用户能力和验收边界 |
| --- | --- |
| Phase 4-1：MIDI 预览与媒体任务 | 复用上传/解析/渲染代码，补齐普通用户可操作的预览与任务状态；验证媒体格式、轮询、取消、失败恢复和重启行为。任务生命周期与输入恢复增量已实现，仍需用户确认客户端轮询体验。先核对第 4 节，不重新造 MIDI 解析器。 |
...
| Phase 4-3：原生证据调查 | 已完成初步样本、日志和两层命令入口核查，下一步追踪原生曲目解析到媒体转发的具体分支；按第 6 节证据门槛推进，详见 [Phase 4-3 文档](./phase4-3-native-webplayer-evidence.md)。 |
