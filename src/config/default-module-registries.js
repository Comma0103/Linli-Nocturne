...
export function createDefaultModuleRegistries({ store = null } = {}) {
  ...
  const harness = new ModuleRegistry('harness')
    .register({ id: 'linli.fusion-v1', version: '1.0.0-exp', label: '林离融合流程（OliviaSoul 检查 + 统一记忆）', create: options => new FusionHarness(options) })
  ...
  const persona = new ModuleRegistry('persona')
    .register({ id: 'linli.persona-bundle', version: '1.0.0-exp', label: '林离完整人格与书信素材包', create: options => new PersonaBundleProvider(options) })
  ...
  const outputPolicy = new ModuleRegistry('outputPolicy')
    .register({ id: 'persona-contract', version: '1.0.0', label: '按所选人格校验落款', create: options => new PersonaReplyPolicy(options) })
    .register({ id: 'none', version: '1.0.0', label: '不校验落款', create: () => new NoopReplyPolicy() });
